...
class FaceRotateConvolution(nn.Module):
    ...
    def __init__(self):
        ...
        self.rotate_mlp = nn.Sequential(
            nn.Conv1d(6, 32, 1),
            nn.InstanceNorm1d(1), #GroupNorm(32x32)
            nn.ReLU(),
            nn.Conv1d(32, 32, 1),
            nn.InstanceNorm1d(1), #tu też, i zamiast instancenorm groupnorm z liczbą kanałów we/wy
            nn.ReLU()
        )
...
class FaceKernelCorrelation(nn.Module):
    ...
    def __init__(self, num_kernel=64, sigma=0.2):
        ...
        self.bn = nn.InstanceNorm1d(num_kernel)
        self.relu = nn.ReLU()
...
class MeshConvolution(nn.Module):
    ...
    def __init__(self, cfg, spatial_in_channel, structural_in_channel, spatial_out_channel, structural_out_channel):
        ...
        self.combination_mlp = nn.Sequential(
            ...
            nn.InstanceNorm1d(self.spatial_out_channel),
            nn.ReLU(),
        )
        ...
        self.aggregation_mlp = nn.Sequential(
            ...
            nn.InstanceNorm1d(self.structural_out_channel),
            nn.ReLU(),
        )
